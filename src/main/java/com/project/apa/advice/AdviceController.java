...
public class AdviceController {
	...
	public String list(String page, Model model, String loginuserseq, String word, String selectedDepartment ) {
		...
		int begin = ((nowPage - 1) * pageSize) + 1; // 페이징 시작 위치
		int end = begin + pageSize - 1; // 페이징 끝 위치
		int n = 0;
		int loop = 0;
		int blockSize = 10;
		...
		if(word == null || word.equals("")) { 
			...
			//쿼리문에서 seqCh 값이 =! if where seq=#{seq}
			
			
			 // 선택한 진료과가 있을 경우 해당하는 의학상담 목록 가져오기
	        if (selectedDepartment != null && !selectedDepartment.isEmpty()) {
	            List<AdviceDTO> filteredList = adviceservice.getFilteredAdviceList(selectedDepartment);
		...
		sb.append((n == 1) ? "<a href='#'!> 이전 | </a>" : String.format("<a href='/apa/advice/list.do?page=%d'> 이전 | </a>", n - 1));
		
		
		
		while (!(loop > blockSize || n > totalPage)) {
            sb.append((n == nowPage) ? String.format(" <a href='#!' style='color:tomato;'>%d</a> ", n) :
                String.format(" <a href='/apa/advice/list.do?page=%d'>%d</a> ", n, n));
            	//String.format(" <a href='/apa/advice/list.do?page=%d&seq=%d'>%d</a> ", n, n, seq));
            loop++;
            n++;
